...
The version in `Cargo.toml` drives npm package publishing. At the end of a work session where `feat:` or `fix:` commits were made:
...
Do NOT bump the version on every individual commit — only once at the end of a session when ready to release.