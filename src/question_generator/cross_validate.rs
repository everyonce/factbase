...
mod tests {
    ...
    async fn test_empty_content_returns_no_questions() {
        let (db, _tmp) = test_db();
        let questions =
            cross_validate_document("", "abc123", &db, &MockEmbedding::new(1024), &MockLlm::default())
                .await
                .unwrap();
    ...
    async fn test_no_list_items_returns_no_questions() {
        ...
        let questions =
            cross_validate_document(content, "abc123", &db, &MockEmbedding::new(1024), &MockLlm::default())
                .await
                .unwrap();
        assert!(questions.is_empty());
    }
