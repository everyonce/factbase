...
pub fn workflow_start(db: &Database, args: &Value) -> Result<Value, FactbaseError> {
    ...
    match workflow.as_str() {
        "update" => Ok(update_step(1, args, &perspective)),
...
fn update_step(step: usize, _args: &Value, perspective: &Option<Perspective>) -> Value {
    ...
    match step {
        ...
        3 => serde_json::json!({
            ...
            "when_done": "Call workflow_next with workflow='update', step=4"
        }),
        4 => serde_json::json!({
            "workflow": "update",
...
fn resolve_step(step: usize, _args: &Value, perspective: &Option<Perspective>) -> Value {
    ...
    match step {
        ...
        3 => serde_json::json!({
            ...
            "when_done": "Call workflow_next with workflow='resolve', step=4"
        }),
        4 => serde_json::json!({
            "workflow": "resolve",
            "step": 4, "total_steps": total,
