...
pub(crate) const DEFAULT_SETUP_SCAN_INSTRUCTION: &str = "Index and verify the new repository. First call scan_repository with time_budget_secs=120 to generate document embeddings, fact embeddings, and detect links. If the response includes `continue: true`, call it again until complete. Then call check_repository with time_budget_secs=120 to see initial quality. If the response includes `continue: true`, call it again with the `checked_pair_ids` array from the response to resume where it left off. Continue calling until complete — do NOT stop early.\n\nReport what the scan found: how many documents were indexed, how many links were detected, and any quality issues from the check.\n\n⚠️ NEXT: When done, you MUST call: workflow(workflow='setup', step=6)";
...
mod tests {
    ...
    fn test_workflow_texts_mention_time_budget_secs() {
        ...
        assert!(check_instr.contains("many iterations"), "update.check should set expectations about iteration count");
    }
