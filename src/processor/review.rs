...
use crate::patterns::{INLINE_QUESTION_MARKER, REVIEW_CALLOUT_HEADER, REVIEW_QUESTION_REGEX, REVIEW_QUEUE_MARKER};
use tracing::warn;
...
pub fn unwrap_review_callout(content: &str) -> (String, bool) {
    ...
    // Find the callout header line
    let mut lines: Vec<&str> = content.lines().collect();
    let callout_start = lines.iter().position(|l| l.trim() == REVIEW_CALLOUT_HEADER);
...
/// wraps it in a collapsed `> [!info]- Review Queue` callout.
...
fn normalize_review_section_inner(content: &str) -> String {
    let Some(marker_pos) = content.find(REVIEW_QUEUE_MARKER) else {
...
"#;
    ...
    fn test_ensure_review_section_callout() {
        ...
        assert!(changed);
        assert!(result.contains("> [!info]- Review Queue"), "Should create callout section");
        assert!(result.contains("> <!-- factbase:review -->"), "Should have callout marker");
    }
