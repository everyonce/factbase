...
pub fn is_citation_specific(text: &str) -> bool {
    if URL_REGEX.is_match(text) { return true; }
    if FILE_PATH_REGEX.is_match(text) { return true; }
    if PAGE_SECTION_REGEX.is_match(text) { return true; }
    if IDENTIFIER_REGEX.is_match(text) { return true; }
    // Named person + date
    if NAMED_PERSON_REGEX.is_match(text) && DATE_REGEX.is_match(text) { return true; }
    // Named channel + date
    if CHANNEL_DATE_REGEX.is_match(text) { return true; }
    false
}
...
mod tests {
    ...
    fn test_book_with_page_is_specific() {
        assert!(is_citation_specific("Peterson Field Guide to Mushrooms of North America, p.247"));
    }

    #[test]
    fn test_email_with_person_date_subject_is_specific() {
        assert!(is_citation_specific("Email from John Smith to Daniel, 2026-02-15, subject \"Q4 Architecture Review\""));
    }

    #[test]
    fn test_slack_channel_thread_date_is_specific() {
        assert!(is_citation_specific("Slack #project-alpha, thread 2026-01-20, re: deployment timeline"));
    }
    ...
    fn test_interview_with_person_date_is_specific() {
        assert!(is_citation_specific("Interview with Jane Doe, CTO of Acme Corp, 2025-11-03"));
    }

    #[test]
    fn test_observation_number_is_specific() {
        assert!(is_citation_specific("iNaturalist observation #12345, 2024-06-15"));
    }
    ...
    fn test_section_reference_is_specific() {
        assert!(is_citation_specific("AWS Well-Architected Framework, Section 3.2"));
    }

    #[test]
    fn test_pp_reference_is_specific() {
        assert!(is_citation_specific("Knuth, The Art of Computer Programming, pp.42-45"));
    }
    ...
    fn test_aws_documentation_with_date_is_vague() {
        assert!(!is_citation_specific("AWS documentation, accessed 2026-03-07"));
    }
