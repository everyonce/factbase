var it=Object.defineProperty;var ot=(e,t,r)=>t in e?it(e,t,{enumerable:!0,configurable:!0,writable:!0,value:r}):e[t]=r;var $e=(e,t,r)=>ot(e,typeof t!="symbol"?t+"":t,r);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function r(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(n){if(n.ep)return;n.ep=!0;const a=r(n);fetch(n.href,a)}})();const ve=[{path:"/",title:"Dashboard",icon:"📊"},{path:"/review",title:"Review Queue",icon:"❓"},{path:"/organize",title:"Organize",icon:"📁"},{path:"/orphans",title:"Orphans",icon:"📝"}];class dt{constructor(){$e(this,"handlers",[]);window.addEventListener("hashchange",()=>this.handleChange()),window.addEventListener("load",()=>this.handleChange())}handleChange(){const t=this.getCurrentRoute();this.handlers.forEach(r=>r(t))}getCurrentRoute(){const t=window.location.hash.slice(1)||"/";return["/","/review","/organize","/orphans"].includes(t)?t:"/"}navigate(t){window.location.hash=t}onRouteChange(t){this.handlers.push(t)}}const Pe=new dt,lt="";class ct{async request(t,r){const s=await fetch(`${lt}${t}`,{headers:{"Content-Type":"application/json"},...r});if(!s.ok){const n=await s.json().catch(()=>({error:`HTTP ${s.status}: ${s.statusText}`,code:"HTTP_ERROR"}));throw new S(n.error,n.code,s.status)}return s.json()}async getStats(){return this.request("/api/stats")}async getReviewStats(){return this.request("/api/stats/review")}async getOrganizeStats(){return this.request("/api/stats/organize")}async getReviewQueue(t){const r=new URLSearchParams;t!=null&&t.repo&&r.set("repo",t.repo),t!=null&&t.type&&r.set("type",t.type);const s=r.toString();return this.request(`/api/review/queue${s?`?${s}`:""}`)}async getDocumentReview(t){return this.request(`/api/review/queue/${encodeURIComponent(t)}`)}async answerQuestion(t,r,s){return this.request(`/api/review/answer/${encodeURIComponent(t)}`,{method:"POST",body:JSON.stringify({question_index:r,answer:s})})}async bulkAnswerQuestions(t){return this.request("/api/review/bulk-answer",{method:"POST",body:JSON.stringify({answers:t})})}async getReviewStatus(){return this.request("/api/review/status")}async getSuggestions(t){const r=new URLSearchParams;t!=null&&t.repo&&r.set("repo",t.repo),t!=null&&t.type&&r.set("type",t.type),(t==null?void 0:t.threshold)!==void 0&&r.set("threshold",t.threshold.toString());const s=r.toString();return this.request(`/api/organize/suggestions${s?`?${s}`:""}`)}async getDocumentSuggestions(t){return this.request(`/api/organize/suggestions/${encodeURIComponent(t)}`)}async dismissSuggestion(t,r,s){return this.request("/api/organize/dismiss",{method:"POST",body:JSON.stringify({type:t,doc_id:r,target_id:s})})}async getOrphans(t){return this.request(`/api/organize/orphans?repo=${encodeURIComponent(t)}`)}async assignOrphan(t,r,s){return this.request("/api/organize/assign-orphan",{method:"POST",body:JSON.stringify({repo:t,line_number:r,target:s})})}async getDocument(t,r){const s=new URLSearchParams;(r==null?void 0:r.include_preview)!==void 0&&s.set("include_preview",r.include_preview.toString()),(r==null?void 0:r.max_content_length)!==void 0&&s.set("max_content_length",r.max_content_length.toString());const n=s.toString();return this.request(`/api/documents/${encodeURIComponent(t)}${n?`?${n}`:""}`)}async getDocumentLinks(t){return this.request(`/api/documents/${encodeURIComponent(t)}/links`)}async getRepositories(){return this.request("/api/repos")}}class S extends Error{constructor(t,r,s){super(t),this.code=r,this.status=s,this.name="ApiRequestError"}get isNotFound(){return this.status===404||this.code==="NOT_FOUND"}get isBadRequest(){return this.status===400||this.code==="BAD_REQUEST"}get isServerError(){return this.status>=500}}const k=new ct;function ut(){return`
  ...
  `}function ie(e){const{title:t="Error",message:r,onRetry:s,retryLabel:n="Retry"}=e,a=s?`retry-${Date.now()}`:null;return`
    <div class="text-center py-8">
      ...
      <p class="font-medium text-red-600 dark:text-red-400">${le(t)}</p>
      <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${le(r)}</p>
      ${a?`
  ...
  `}function oe(e){const t=document.querySelectorAll('[id^="retry-"]'),r=t[t.length-1];r==null||r.addEventListener("click",e)}function le(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}const u={stats:null,review:null,organize:null,loading:!0,error:null,autoRefresh:!1,refreshInterval:null},mt=3e4;function pt(e){return e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(1)} MB`}function bt(e){return e?new Date(e).toLocaleString():"Never"}async function se(){u.loading=!0,u.error=null,Le();try{const[e,t,r]=await Promise.all([k.getStats(),k.getReviewStats(),k.getOrganizeStats()]);u.stats=e,u.review=t,u.organize=r}catch(e){e instanceof S?u.error=e.message:u.error="Failed to load dashboard data"}finally{u.loading=!1,Le()}}function Le(){var a,i;const e=document.getElementById("review-count");e&&(e.textContent=u.loading?"...":((a=u.review)==null?void 0:a.unanswered.toString())??"-");const t=document.getElementById("organize-count");if(t){const d=u.organize?u.organize.merge_candidates+u.organize.misplaced_candidates:0;t.textContent=u.loading?"...":d.toString()}const r=document.getElementById("orphan-count");r&&(r.textContent=u.loading?"...":((i=u.organize)==null?void 0:i.orphan_count.toString())??"-");const s=document.getElementById("stats-content");s&&(u.loading?s.innerHTML=gt():u.error?(s.innerHTML=ie({title:"Error loading stats",message:u.error,onRetry:se}),oe(se)):u.stats&&(s.innerHTML=`
      ...
      `));const n=document.getElementById("auto-refresh-toggle");n&&(n.checked=u.autoRefresh)}function xt(e){u.autoRefresh=e,e&&!u.refreshInterval?u.refreshInterval=window.setInterval(()=>se(),mt):!e&&u.refreshInterval&&(clearInterval(u.refreshInterval),u.refreshInterval=null)}function Ee(){return`
  ...
  `}function ft(){const e=document.getElementById("auto-refresh-toggle");e==null||e.addEventListener("change",t=>{xt(t.target.checked)}),se()}function yt(){u.refreshInterval&&(clearInterval(u.refreshInterval),u.refreshInterval=null)}const N=new Map;function Fe(e,t){return`${e}:${t}`}function Ne(e,t){const r=Fe(e,t);return N.has(r)||N.set(r,{submitting:!1,error:null}),N.get(r)}function T(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function vt(e,t,r){const s=Ne(e,t),n=`answer-form-${T(e)}-${t}`,a=`answer-input-${T(e)}-${t}`,i=`answer-label-${T(e)}-${t}`,d=`answer-hint-${T(e)}-${t}`,h=ht(r),L=s.submitting?"opacity-50 pointer-events-none":"";return`
    <form id="${n}" class="answer-form mt-3 ${L}" data-doc-id="${T(e)}" data-question-index="${t}">
      <div class="space-y-2">
        ...
        ></textarea>
        <div id="${d}" class="sr-only">Press Ctrl+Enter to submit. Use Dismiss to skip or Delete fact to remove.</div>
        <div class="flex items-center justify-between">
  ...
  `}function ht(e){switch(e){case"temporal":return'e.g., "Started March 2022, left December 2024"';case"conflict":return'e.g., "Both were part-time, no conflict" or explain resolution';case"missing":return'e.g., "LinkedIn profile, checked 2024-01-15"';case"ambiguous":return'e.g., "Home address" or "split: home in Austin, work in SF"';case"stale":return'e.g., "Still accurate as of today" or provide update';case"duplicate":return'e.g., "Keep this one" or "Merge into [other_id]"';default:return"Enter your answer..."}}async function ce(e,t,r,s){const n=Ne(e,t);n.submitting=!0,n.error=null;try{await k.answerQuestion(e,t,r),s.onSuccess(e,t,r),N.delete(Fe(e,t))}catch(a){a instanceof S?n.error=a.message:n.error="Failed to submit answer",s.onError(n.error)}finally{n.submitting=!1}}function kt(e,t){e.addEventListener("submit",async r=>{const s=r.target.closest(".answer-form");if(!s)return;r.preventDefault();const n=s.dataset.docId,a=parseInt(s.dataset.questionIndex||"0",10),i=s.querySelector("textarea"),d=(i==null?void 0:i.value.trim())||"";!n||!d||await ce(n,a,d,t)}),e.addEventListener("click",async r=>{const s=r.target.closest(".quick-action");if(!s)return;const n=s.closest(".answer-form");if(!n)return;const a=n.dataset.docId,i=parseInt(n.dataset.questionIndex||"0",10),d=s.dataset.action;if(!a||!d)return;await ce(a,i,d==="dismiss"?"dismiss":"delete",t)}),e.addEventListener("keydown",async r=>{if(r.key==="Enter"&&(r.ctrlKey||r.metaKey)){const s=r.target;if(s.tagName!=="TEXTAREA")return;const n=s.closest(".answer-form");if(!n)return;r.preventDefault();const a=n.dataset.docId,i=parseInt(n.dataset.questionIndex||"0",10),d=s.value.trim();if(!a||!d)return;await ce(a,i,d,t)}})}function wt(){N.clear()}const g={selections:new Set,submitting:!1,showBulkAnswer:!1};function he(e,t){return`${e}:${t}`}function Ue(e){const[t,r]=e.split(":");return{docId:t,questionIndex:parseInt(r,10)}}function ue(){return Array.from(g.selections).map(Ue)}function $t(e,t){const r=he(e,t);g.selections.has(r)?g.selections.delete(r):g.selections.add(r)}function Lt(e){g.selections.clear();for(const t of e)for(let r=0;r<t.questions.length;r++)t.questions[r].answered||g.selections.add(he(t.doc_id,r))}function Et(){g.selections.clear()}function Qe(){g.selections.clear(),g.submitting=!1,g.showBulkAnswer=!1}function Se(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function St(e){const t=g.selections.size,r=t>0;return`
  ...
  `}function Mt(e,t,r){if(r.answered)return"";const s=he(e,t),n=g.selections.has(s);return`
    <input
      type="checkbox"
      id="${`bulk-checkbox-${Se(e)}-${t}`}"
      ...
      ${n?"checked":""}
    />
  `}async function Ce(e,t,r){if(g.selections.size!==0){g.submitting=!0;try{const s=Array.from(g.selections).map(i=>{const{docId:d,questionIndex:h}=Ue(i);return{doc_id:d,question_index:h,answer:t}}),n=await k.bulkAnswerQuestions(s);n.errors&&n.errors.length>0&&r.onError(`Some answers failed: ${n.errors.join(", ")}`);const a=n.results.filter(i=>i.success).length;a>0&&(r.onSuccess(a,e==="dismiss"?"dismissed":"answered"),g.selections.clear(),g.showBulkAnswer=!1)}catch(s){s instanceof S?r.onError(s.message):r.onError("Failed to process bulk action")}finally{g.submitting=!1}}}function Tt(e,t,r,s){var n,a,i,d,h,L;(n=e.querySelector("#bulk-select-all"))==null||n.addEventListener("click",()=>{Lt(t),r.onSelectionChange(ue()),s()}),(a=e.querySelector("#bulk-select-none"))==null||a.addEventListener("click",()=>{Et(),r.onSelectionChange(ue()),s()}),(i=e.querySelector("#bulk-dismiss-btn"))==null||i.addEventListener("click",async()=>{if(g.selections.size===0)return;const E=g.selections.size;confirm(`Dismiss ${E} selected question(s)?`)&&(await Ce("dismiss","dismiss",r),s())}),(d=e.querySelector("#bulk-answer-btn"))==null||d.addEventListener("click",()=>{g.submitting||(g.showBulkAnswer=!g.showBulkAnswer,s())}),(h=e.querySelector("#bulk-answer-cancel"))==null||h.addEventListener("click",()=>{g.showBulkAnswer=!1,s()}),(L=e.querySelector("#bulk-answer-submit"))==null||L.addEventListener("click",async()=>{const E=e.querySelector("#bulk-answer-input"),C=E==null?void 0:E.value.trim();C&&(await Ce("answer",C,r),s())}),e.addEventListener("change",E=>{const C=E.target;if(!C.classList.contains("bulk-checkbox"))return;const we=C.dataset.docId,at=parseInt(C.dataset.questionIndex||"0",10);we&&($t(we,at),r.onSelectionChange(ue()),s())})}const It={temporal:{bg:"bg-blue-100 dark:bg-blue-900",text:"text-blue-700 dark:text-blue-200"},conflict:{bg:"bg-red-100 dark:bg-red-900",text:"text-red-700 dark:text-red-200"},missing:{bg:"bg-amber-100 dark:bg-amber-900",text:"text-amber-700 dark:text-amber-200"},ambiguous:{bg:"bg-purple-100 dark:bg-purple-900",text:"text-purple-700 dark:text-purple-200"},stale:{bg:"bg-gray-100 dark:bg-gray-700",text:"text-gray-700 dark:text-gray-200"},duplicate:{bg:"bg-green-100 dark:bg-green-900",text:"text-green-700 dark:text-green-200"}};function F(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function Ke(e){const t=It[e]||{bg:"bg-gray-100 dark:bg-gray-700",text:"text-gray-700 dark:text-gray-200"};return`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${t.bg} ${t.text}">@q[${F(e)}]</span>`}function At(e,t,r,s={}){const{showAnswerForm:n=!0,showCheckbox:a=!1}=s,i=e.answered?"opacity-60":"",d=e.answered?'<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200 ml-2">Answered</span>':"",h=a?Mt(t,r,e):"",L=e.line_ref?`<button
      ...
      >Line ${e.line_ref}</button>`:"",E=e.answered&&e.answer?`<div class="mt-2 p-2 bg-gray-50 dark:bg-gray-800 rounded text-sm text-gray-600 dark:text-gray-400">
        <span class="font-medium">Answer:</span> ${F(e.answer)}
      </div>`:n&&!e.answered?vt(t,r,e.question_type):"";return`
    <div class="question-card border border-gray-200 dark:border-gray-700 rounded-lg p-4 ${i}" data-doc-id="${F(t)}" data-question-index="${r}">
      ...
      <p class="mt-2 text-gray-700 dark:text-gray-300">${F(e.description)}</p>
      ${E}
    </div>
  `}const v={loading:!1,error:null,document:null,highlightLine:null};let Q=!1,m=null;function R(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function Me(e){return!e||e.length===0?'<p class="text-sm text-gray-500 dark:text-gray-400">None</p>':`
    <ul class="space-y-1">
      ${e.map(t=>`
        <li>
          <button
  ...
  `}function Rt(e,t){return e.split(`
`).map((s,n)=>{const a=n+1,i=t!==null&&a===t;return`
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 overflow-auto p-4 font-mono bg-white dark:bg-gray-900" role="region" aria-label="Document content">
        ${e.content?Rt(e.content,v.highlightLine):'<p class="text-gray-500 dark:text-gray-400">No content available</p>'}
      </div>
    </div>
  `}function Te(){if(!m)return;const e=m.querySelector("#preview-panel-content");e&&(e.innerHTML=Ge(),We(),v.highlightLine!==null&&!v.loading&&setTimeout(()=>{const t=document.getElementById("highlighted-line");t==null||t.scrollIntoView({behavior:"smooth",block:"center"})},100))}function We(){var e,t;(e=document.getElementById("preview-close-btn"))==null||e.addEventListener("click",me),(t=document.getElementById("preview-close-error"))==null||t.addEventListener("click",me),document.querySelectorAll(".preview-link-btn").forEach(r=>{r.addEventListener("click",s=>{const n=s.currentTarget.dataset.docId;n&&K(n)})})}async function _t(e){v.loading=!0,v.error=null,v.document=null,Te();try{const t=await k.getDocument(e),r=await k.getDocumentLinks(e);v.document={...t,links_to:r.links_to,linked_from:r.linked_from}}catch(t){t instanceof S?v.error=t.message:v.error="Failed to load document"}finally{v.loading=!1,Te()}}function K(e,t){v.highlightLine=t??null,Q||(qt(),Q=!0),_t(e)}function me(){m&&(m.classList.add("translate-x-full"),setTimeout(()=>{m==null||m.remove(),m=null},300)),Q=!1,v.document=null,v.error=null,v.highlightLine=null}function qt(){m==null||m.remove(),m=document.createElement("div"),m.id="document-preview-panel",m.className=`
  ...
  `,document.body.appendChild(m),requestAnimationFrame(()=>{m==null||m.classList.remove("translate-x-full")}),We();const e=t=>{t.key==="Escape"&&Q&&me()};document.addEventListener("keydown",e),m._cleanup=()=>{document.removeEventListener("keydown",e)}}function ke(){if(m){const e=m._cleanup;e&&e(),m.remove(),m=null}Q=!1,v.document=null,v.error=null,v.highlightLine=null}const B=[];let pe="toast-container";function Ve(){if(document.getElementById(pe))return;const e=document.createElement("div");e.id=pe,e.className="fixed bottom-4 right-4 z-50 flex flex-col space-y-2 max-w-sm",e.setAttribute("role","region"),e.setAttribute("aria-label","Notifications"),document.body.appendChild(e)}function Z(e){Ve();const t=`toast-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,r=e.duration??5e3,s={id:t,options:e};return r>0&&(s.timeoutId=window.setTimeout(()=>be(t),r)),B.push(s),Je(),t}function be(e){const t=B.findIndex(s=>s.id===e);if(t===-1)return;const r=B[t];r.timeoutId&&clearTimeout(r.timeoutId),B.splice(t,1),Je()}const M={success:(e,t)=>Z({message:e,type:"success",...t}),error:(e,t)=>Z({message:e,type:"error",duration:0,...t}),info:(e,t)=>Z({message:e,type:"info",...t}),warning:(e,t)=>Z({message:e,type:"warning",...t})};function Je(){const e=document.getElementById(pe);e&&(e.innerHTML=B.map(t=>Bt(t)).join(""),B.forEach(t=>{const r=document.getElementById(`${t.id}-dismiss`);if(r==null||r.addEventListener("click",()=>be(t.id)),t.options.action){const s=document.getElementById(`${t.id}-action`);s==null||s.addEventListener("click",()=>{var n;(n=t.options.action)==null||n.onClick(),be(t.id)})}}))}function Bt(e){const{id:t,options:r}=e,{message:s,type:n="info",action:a}=r,i=jt(n),d=Ht(n);return`
    <div
      id="${t}"
    ...
    >
      ...
      <div class="ml-3 flex-1">
        <p class="text-sm font-medium ${i.text}">${Ie(s)}</p>
        ${a?`
          <button
            id="${t}-action"
  ...
  `}function jt(e){switch(e){case"success":return{bg:"bg-green-50 dark:bg-green-900/30",border:"border-green-200 dark:border-green-800",text:"text-green-800 dark:text-green-200",icon:"text-green-500 dark:text-green-400",action:"text-green-700 dark:text-green-300",dismiss:"text-green-500 dark:text-green-400"};case"error":return{bg:"bg-red-50 dark:bg-red-900/30",border:"border-red-200 dark:border-red-800",text:"text-red-800 dark:text-red-200",icon:"text-red-500 dark:text-red-400",action:"text-red-700 dark:text-red-300",dismiss:"text-red-500 dark:text-red-400"};case"warning":return{bg:"bg-amber-50 dark:bg-amber-900/30",border:"border-amber-200 dark:border-amber-800",text:"text-amber-800 dark:text-amber-200",icon:"text-amber-500 dark:text-amber-400",action:"text-amber-700 dark:text-amber-300",dismiss:"text-amber-500 dark:text-amber-400"};case"info":default:return{bg:"bg-blue-50 dark:bg-blue-900/30",border:"border-blue-200 dark:border-blue-800",text:"text-blue-800 dark:text-blue-200",icon:"text-blue-500 dark:text-blue-400",action:"text-blue-700 dark:text-blue-300",dismiss:"text-blue-500 dark:text-blue-400"}}}function Ht(e){switch(e){case"success":return`
      ...
      `}}function Ie(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}const l={data:null,repos:[],loading:!0,error:null,filterRepo:"",filterType:"",successMessage:null,bulkMode:!1},Dt=["temporal","conflict","missing","ambiguous","stale","duplicate"];function U(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}async function H(){l.loading=!0,l.error=null,_();try{const e={};l.filterRepo&&(e.repo=l.filterRepo),l.filterType&&(e.type=l.filterType);const[t,r]=await Promise.all([k.getReviewQueue(e),l.repos.length===0?k.getRepositories():Promise.resolve({repositories:l.repos})]);l.data=t,l.repos=r.repositories}catch(e){e instanceof S?l.error=e.message:l.error="Failed to load review queue"}finally{l.loading=!1,_()}}function zt(e){const t=e.questions.filter(s=>!s.answered).length,r=e.questions.length;return`
    <div class="document-group bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
      <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">${U(e.doc_title)}</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">${U(e.file_path)}</p>
          </div>
          <div class="flex items-center space-x-3">
            <button
      ...
      <div class="p-4 space-y-3">
        ${e.questions.map((s,n)=>At(s,e.doc_id,n,{showAnswerForm:!l.bulkMode,showCheckbox:l.bulkMode})).join("")}
      </div>
    </div>
  `}function Pt(){const e=l.repos.map(r=>`<option value="${U(r.id)}" ${l.filterRepo===r.id?"selected":""}>${U(r.name)}</option>`).join(""),t=Dt.map(r=>`<option value="${r}" ${l.filterType===r?"selected":""}>${r}</option>`).join("");return`
  ...
  `}function Ye(){if(!l.data)return"";const e={};for(const r of l.data.documents)for(const s of r.questions)s.answered||(e[s.question_type]=(e[s.question_type]||0)+1);return`
    <div class="flex items-center justify-between text-sm">
      ...
      <div class="text-gray-500 dark:text-gray-400">
        ${l.data.unanswered} pending / ${l.data.total} total
      </div>
    </div>
  `}function _(){const e=document.getElementById("review-queue-content");if(!e)return;const t=document.getElementById("review-summary");t&&l.data&&(t.innerHTML=Ye());const r=document.getElementById("bulk-actions-container");r&&l.data&&l.bulkMode?(r.innerHTML=St(l.data.unanswered),Tt(r,l.data.documents,{onSuccess:Qt,onError:Kt,onSelectionChange:Gt},_)):r&&(r.innerHTML="");const s=document.getElementById("review-message");if(s&&(s.innerHTML=""),l.loading){e.innerHTML=`
      <div class="space-y-4">
        ${de()}
        ${de()}
        ${de()}
      </div>
    `;return}if(l.error){e.innerHTML=ie({title:"Error loading review queue",message:l.error,onRetry:H}),oe(H);return}if(!l.data||l.data.documents.length===0){e.innerHTML=`
      <div class="text-center py-8">
    ...
    `;return}const n=[...l.data.documents].sort((a,i)=>{const d=a.questions.filter(L=>!L.answered).length;return i.questions.filter(L=>!L.answered).length-d});e.innerHTML=`
    <div class="space-y-4">
      ${n.map(a=>zt(a)).join("")}
    </div>
  `,l.bulkMode||kt(e,{onSuccess:Nt,onError:Ut}),Ot(e)}function Ot(e){e.querySelectorAll(".preview-doc-btn").forEach(t=>{t.addEventListener("click",r=>{const s=r.currentTarget.dataset.docId;s&&K(s)})}),e.querySelectorAll(".preview-line-btn").forEach(t=>{t.addEventListener("click",r=>{const s=r.currentTarget.dataset.docId,n=r.currentTarget.dataset.lineRef;s&&K(s,n?parseInt(n,10):void 0)})})}function Ft(){const e=document.getElementById("filter-repo"),t=document.getElementById("filter-type");e==null||e.addEventListener("change",r=>{l.filterRepo=r.target.value,H()}),t==null||t.addEventListener("change",r=>{l.filterType=r.target.value,H()})}function Nt(e,t,r){if(l.data){const s=l.data.documents.find(n=>n.doc_id===e);s&&s.questions[t]&&(s.questions[t].answered=!0,s.questions[t].answer=r,l.data.answered++,l.data.unanswered--)}M.success(`Answer submitted for question ${t+1}`),_()}function Ut(e){M.error(`Failed to submit answer: ${e}`),_()}function Qt(e,t){M.success(`Successfully ${t} ${e} question(s)`),H()}function Kt(e){M.error(`Bulk action failed: ${e}`),_()}function Gt(e){}function Wt(){l.bulkMode=!l.bulkMode,l.bulkMode||Qe(),_()}function Vt(){const e=l.bulkMode?"Exit bulk mode":"Bulk actions";return`
    <div class="space-y-4 sm:space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Review Queue</h2>
        <button
          id="bulk-mode-toggle"
          class="inline-flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md ${l.bulkMode?"bg-blue-600 text-white hover:bg-blue-700":"bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"}"
        >
          ${e}
        </button>
      </div>
  ...
  `}function Jt(){Ft(),Yt(),H()}function Yt(){var e;(e=document.getElementById("bulk-mode-toggle"))==null||e.addEventListener("click",Wt)}function Xt(){l.data=null,l.loading=!0,l.error=null,l.successMessage=null,l.bulkMode=!1,wt(),Qe(),ke()}function y(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function D(e){const t={merge:"bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-200",misplaced:"bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200"},r={merge:"🔗",misplaced:"📁"};return`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${t[e]}">
  ...
  `}const f={loading:!1,error:null,doc1:null,doc2:null};let G=!1,b=null,V=null,J=null;function A(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function xe(e){const t=e.split(`
`);let r=0;for(const s of t){const n=s.trim();(n.startsWith("- ")||n.startsWith("* ")||/^\d+\.\s/.test(n))&&r++}return r}function Ae(e,t){var n;if(!e)return`
      <div class="flex-1 min-w-0 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
        <div class="text-center text-gray-500 dark:text-gray-400">
          Loading...
        </div>
      </div>
    `;const r=e.content?xe(e.content):0,s=((n=e.content)==null?void 0:n.split(`
`))||[];return`
    <div class="flex-1 min-w-0 flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      <div class="flex-shrink-0 p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
        ...
        <h4 class="mt-1 text-sm font-semibold text-gray-900 dark:text-white truncate" title="${A(e.title)}">
          ${A(e.title)}
        </h4>
        <div class="mt-1 flex items-center space-x-2">
          ...
          <span class="text-xs text-gray-400 dark:text-gray-500">[${A(e.id)}]</span>
        </div>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 truncate" title="${A(e.file_path)}">
          ${A(e.file_path)}
        </p>
      </div>
      <!-- Content -->
      <div class="flex-1 overflow-auto p-3 font-mono text-xs">
        ${s.map((a,i)=>`
          <div class="flex hover:bg-gray-50 dark:hover:bg-gray-700/50">
            ...
            <pre class="flex-1 whitespace-pre-wrap break-words text-gray-700 dark:text-gray-300">${A(a)||" "}</pre>
          </div>
        `).join("")}
      </div>
    </div>
  `}function Xe(){var s,n;if(f.loading)return`
    ...
    `;if(f.error)return`
      <div class="p-4 text-center">
        <p class="text-red-600 dark:text-red-400">${A(f.error)}</p>
        <button id="merge-preview-close-error" class="mt-2 text-sm text-blue-600 dark:text-blue-400 hover:underline">
          Close
        </button>
      </div>
    `;const e=(s=f.doc1)!=null&&s.content?xe(f.doc1.content):0,t=(n=f.doc2)!=null&&n.content?xe(f.doc2.content):0;return`
    <div class="flex flex-col h-full">
      <!-- Header -->
      <div class="flex-shrink-0 p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
      ...
      <div class="flex-1 overflow-hidden p-4">
        <div class="flex gap-4 h-full">
          ${Ae(f.doc1,"Document 1")}
          ${Ae(f.doc2,"Document 2")}
        </div>
      </div>
      ...
      <div class="flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
        <div class="flex items-center justify-between">
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Merge requires CLI: <code class="bg-gray-100 dark:bg-gray-700 px-1 rounded">factbase organize merge ${V||"doc1"} ${J||"doc2"}</code>
          </p>
          <div class="flex items-center space-x-2">
            <button
  ...
  `}function Re(){if(!b)return;const e=b.querySelector("#merge-preview-content");e&&(e.innerHTML=Xe(),Ze())}function Ze(){var e,t,r,s;(e=document.getElementById("merge-preview-close-btn"))==null||e.addEventListener("click",te),(t=document.getElementById("merge-preview-close-error"))==null||t.addEventListener("click",te),(r=document.getElementById("merge-preview-dismiss-btn"))==null||r.addEventListener("click",()=>{te()}),(s=document.getElementById("merge-preview-approve-btn"))==null||s.addEventListener("click",()=>{const n=`factbase organize merge ${V} ${J}`;navigator.clipboard.writeText(n).then(()=>{const a=document.getElementById("merge-preview-approve-btn");a&&(a.textContent="Copied!",setTimeout(()=>{a.textContent="Copy Command"},2e3))}).catch(()=>{alert(`Run: ${n}`)})})}async function sr(e,t){f.loading=!0,f.error=null,f.doc1=null,f.doc2=null,Re();try{const[r,s]=await Promise.all([k.getDocument(e),k.getDocument(t)]);f.doc1=r,f.doc2=s}catch(r){r instanceof S?f.error=r.message:f.error="Failed to load documents"}finally{f.loading=!1,Re()}}function nr(e,t){V=e,J=t,G||(ar(),G=!0),sr(e,t)}function te(){b&&(b.classList.add("translate-x-full"),setTimeout(()=>{b==null||b.remove(),b=null},300)),G=!1,f.doc1=null,f.doc2=null,f.error=null,V=null,J=null}function ar(){b==null||b.remove(),b=document.createElement("div"),b.id="merge-preview-panel",b.className=`
  ...
  `,document.body.appendChild(b),requestAnimationFrame(()=>{b==null||b.classList.remove("translate-x-full")}),Ze();const e=t=>{t.key==="Escape"&&G&&te()};document.addEventListener("keydown",e),b._cleanup=()=>{document.removeEventListener("keydown",e)}}function ir(){if(b){const e=b._cleanup;e&&e(),b.remove(),b=null}G=!1,f.doc1=null,f.doc2=null,f.error=null,V=null,J=null}const p={loading:!1,error:null,doc:null,sections:[]};let W=!1,x=null,Y=null;function q(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function _e(e){const t=e.split(`
`);let r=0;for(const s of t){const n=s.trim();(n.startsWith("- ")||n.startsWith("* ")||/^\d+\.\s/.test(n))&&r++}return r}function or(e){const t=e.split(`
`),r=[];let s="Introduction",n=0,a=1,i=[];for(let d=0;d<t.length;d++){const h=d+1,L=t[d];if(L.startsWith("<!-- factbase:"))continue;const E=dr(L);if(E){if(i.length>0){const C=i.join(`
`).trim();C&&r.push({title:s,level:n,startLine:a,endLine:h-1,content:C,factCount:_e(C)})}s=E.title,n=E.level,a=h,i=[]}else i.push(L)}if(i.length>0){const d=i.join(`
`).trim();d&&r.push({title:s,level:n,startLine:a,endLine:t.length,content:d,factCount:_e(d)})}return r}function dr(e){const t=e.trimStart();if(!t.startsWith("#"))return null;let r=0;for(const n of t)if(n==="#")r++;else break;if(r===0||r>6)return null;const s=t.slice(r).trim();return s?{level:r,title:s}:null}function lr(e,t){const r=e.level>0?`<span class="text-xs text-gray-400 dark:text-gray-500">H${e.level}</span>`:'<span class="text-xs text-gray-400 dark:text-gray-500">Intro</span>',s=e.content.split(`
...
`).length>5;return`
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      <div class="p-3 font-mono text-xs text-gray-600 dark:text-gray-400 max-h-32 overflow-hidden">
        ${s.map(a=>`<div class="truncate">${q(a)||"&nbsp;"}</div>`).join("")}
  ...
  `}function et(){if(p.loading)return`
    ...
    `;if(p.error)return`
      <div class="p-4 text-center">
        <p class="text-red-600 dark:text-red-400">${q(p.error)}</p>
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex-shrink-0 p-4 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            <h4 class="text-sm font-semibold text-gray-900 dark:text-white">${q(p.doc.title)}</h4>
            <div class="mt-1 flex items-center space-x-2">
              <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                ${q(p.doc.doc_type)}
              </span>
              <span class="text-xs text-gray-400 dark:text-gray-500">[${q(p.doc.id)}]</span>
            </div>
          </div>
      ...
      <div class="flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
        <div class="flex items-center justify-between">
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Split requires CLI: <code class="bg-gray-100 dark:bg-gray-700 px-1 rounded">factbase organize split ${Y||"doc_id"}</code>
          </p>
          <div class="flex items-center space-x-2">
            <button
  ...
  `}function qe(){if(!x)return;const e=x.querySelector("#split-preview-content");e&&(e.innerHTML=et(),tt())}function tt(){var e,t,r,s;(e=document.getElementById("split-preview-close-btn"))==null||e.addEventListener("click",re),(t=document.getElementById("split-preview-close-error"))==null||t.addEventListener("click",re),(r=document.getElementById("split-preview-dismiss-btn"))==null||r.addEventListener("click",re),(s=document.getElementById("split-preview-copy-btn"))==null||s.addEventListener("click",()=>{const n=`factbase organize split ${Y}`;navigator.clipboard.writeText(n).then(()=>{const a=document.getElementById("split-preview-copy-btn");a&&(a.textContent="Copied!",setTimeout(()=>{a.textContent="Copy Command"},2e3))}).catch(()=>{alert(`Run: ${n}`)})})}async function cr(e){p.loading=!0,p.error=null,p.doc=null,p.sections=[],qe();try{const t=await k.getDocument(e);p.doc=t,t.content&&(p.sections=or(t.content))}catch(t){t instanceof S?p.error=t.message:p.error="Failed to load document"}finally{p.loading=!1,qe()}}function ur(e){Y=e,W||(gr(),W=!0),cr(e)}function re(){x&&(x.classList.add("translate-x-full"),setTimeout(()=>{x==null||x.remove(),x=null},300)),W=!1,p.doc=null,p.sections=[],p.error=null,Y=null}function gr(){x==null||x.remove(),x=document.createElement("div"),x.id="split-preview-panel",x.className=`
    fixed top-0 right-0 h-full w-full sm:w-[480px] lg:w-[560px]
    bg-white dark:bg-gray-800 shadow-xl
    transform translate-x-full transition-transform duration-300 ease-in-out
    z-50 flex flex-col
  ...
  `,document.body.appendChild(x),requestAnimationFrame(()=>{x==null||x.classList.remove("translate-x-full")}),tt();const e=t=>{t.key==="Escape"&&W&&re()};document.addEventListener("keydown",e),x._cleanup=()=>{document.removeEventListener("keydown",e)}}function mr(){if(x){const e=x._cleanup;e&&e(),x.remove(),x=null}W=!1,p.doc=null,p.sections=[],p.error=null,Y=null}const c={data:null,repos:[],loading:!0,error:null,filterRepo:"",filterType:"",successMessage:null},pr=["merge","misplaced"];function Be(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}async function z(){c.loading=!0,c.error=null,fe();try{const e={};c.filterRepo&&(e.repo=c.filterRepo),c.filterType&&(e.type=c.filterType);const[t,r]=await Promise.all([k.getSuggestions(e),c.repos.length===0?k.getRepositories():Promise.resolve({repositories:c.repos})]);c.data=t,c.repos=r.repositories}catch(e){e instanceof S?c.error=e.message:c.error="Failed to load suggestions"}finally{c.loading=!1,fe()}}function br(){const e=c.repos.map(r=>`<option value="${Be(r.id)}" ${c.filterRepo===r.id?"selected":""}>${Be(r.name)}</option>`).join(""),t=pr.map(r=>`<option value="${r}" ${c.filterType===r?"selected":""}>${r}</option>`).join("");return`
    <div class="flex flex-col sm:flex-row gap-4">
      <div class="flex-1 sm:flex-none">
        <label for="filter-repo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Repository</label>
        <select id="filter-repo" class="block w-full sm:w-48 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
  ...
  `}function rt(){if(!c.data)return"";const e=c.data.merge.length,t=c.data.misplaced.length,r=[];return e>0&&r.push(`${D("merge")} <span class="ml-1 text-gray-600 dark:text-gray-400">${e}</span>`),t>0&&r.push(`${D("misplaced")} <span class="ml-1 text-gray-600 dark:text-gray-400">${t}</span>`),`
    <div class="flex items-center justify-between text-sm">
      <div class="flex items-center space-x-4">
        ${r.length>0?r.join('<span class="mx-2 text-gray-300 dark:text-gray-600">|</span>'):'<span class="text-gray-500 dark:text-gray-400">No pending suggestions</span>'}
      </div>
      <div class="text-gray-500 dark:text-gray-400">
        ${c.data.total} total suggestion${c.data.total!==1?"s":""}
      </div>
    </div>
  `}function xr(){return!c.data||c.data.merge.length===0||c.filterType&&c.filterType!=="merge"?"":`
    <div class="space-y-4">
      ...
      <div class="space-y-3">
        ${c.data.merge.map((e,t)=>tr(e,t)).join("")}
      </div>
    </div>
  `}function fr(){return!c.data||c.data.misplaced.length===0||c.filterType&&c.filterType!=="misplaced"?"":`
    <div class="space-y-4">
      <h3 class="text-lg font-medium text-gray-900 dark:text-white flex items-center space-x-2">
        ${D("misplaced")}
      ...
      <div class="space-y-3">
        ${c.data.misplaced.map((e,t)=>rr(e,t)).join("")}
      </div>
    </div>
  `}function fe(){const e=document.getElementById("organize-content");if(!e)return;const t=document.getElementById("organize-summary");t&&c.data&&(t.innerHTML=rt());const r=document.getElementById("organize-message");if(r&&(r.innerHTML=""),c.loading){e.innerHTML=Oe(4);return}if(c.error){e.innerHTML=ie({title:"Error loading suggestions",message:c.error,onRetry:z}),oe(z);return}if(!c.data||c.data.total===0){e.innerHTML=`
      <div class="text-center py-8">
        <span class="text-4xl">✅</span>
    ...
    `;return}const s=xr(),n=fr();if(!s&&!n){e.innerHTML=`
      <div class="text-center py-8">
        <span class="text-4xl">🔍</span>
  ...
  `}function wr(){yr(),z()}function $r(){c.data=null,c.loading=!0,c.error=null,c.successMessage=null,ke(),ir(),mr()}function I(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function Lr(e,t,r={}){const{showCheckbox:s=!1}=r,n=`orphan-${t}-${e.line_number}`,a=e.source_doc?`<span class="text-xs text-gray-500 dark:text-gray-400">from <button class="preview-source-btn text-blue-600 dark:text-blue-400 hover:underline" data-doc-id="${I(e.source_doc)}" data-line="${e.source_line||""}">${I(e.source_doc)}</button>${e.source_line?` line ${e.source_line}`:""}</span>`:"",i=e.answered?`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">
        Assigned: ${I(e.answer||"")}
        ...
        Pending
      </span>`,d=s?`<input type="checkbox" id="${n}" class="orphan-checkbox h-4 w-4 text-blue-600 rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-blue-500" data-repo="${I(t)}" data-line="${e.line_number}">`:"",h=e.answered?"":`<div class="orphan-assign-form mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
        <div class="flex items-center space-x-2">
          <input
            type="text"
  ...
  `:""}function Cr(){return!o.bulkMode||o.selectedLines.size===0?"":`
  ...
  `}function j(){const e=document.getElementById("orphans-content");if(!e)return;const t=document.getElementById("orphans-summary");t&&o.data&&(t.innerHTML=st());const r=document.getElementById("bulk-actions-container");r&&(r.innerHTML=Cr(),Tr());const s=document.getElementById("orphans-message");if(s&&(s.innerHTML=""),o.loading){e.innerHTML=Oe(3);return}if(o.error){e.innerHTML=ie({title:"Error loading orphans",message:o.error,onRetry:P}),oe(P);return}if(!o.selectedRepo){e.innerHTML=`
      <div class="text-center py-8">
    ...
    <div class="space-y-3">
      ${n.map(a=>Lr(a,o.selectedRepo,{showCheckbox:o.bulkMode})).join("")}
    </div>
  `,Mr(e)}function Mr(e){e.querySelectorAll(".orphan-assign-btn").forEach(t=>{t.addEventListener("click",async r=>{const s=r.currentTarget,n=parseInt(s.dataset.line||"0",10),a=s.closest(".orphan-card"),i=a==null?void 0:a.querySelector(".orphan-target-input"),d=a==null?void 0:a.querySelector(".orphan-error");if(!(i!=null&&i.value.trim())){d&&(d.textContent='Please enter a document ID or "dismiss"',d.classList.remove("hidden"));return}s.disabled=!0,s.textContent="Assigning...";try{await He(n,i.value.trim())}catch(h){d&&(d.textContent=h instanceof Error?h.message:"Failed to assign",d.classList.remove("hidden")),s.disabled=!1,s.textContent="Assign"}})}),e.querySelectorAll(".orphan-dismiss-btn").forEach(t=>{t.addEventListener("click",async r=>{const s=r.currentTarget,n=parseInt(s.dataset.line||"0",10),a=s.closest(".orphan-card"),i=a==null?void 0:a.querySelector(".orphan-error");s.disabled=!0,s.textContent="Dismissing...";try{await He(n,"dismiss")}catch(d){i&&(i.textContent=d instanceof Error?d.message:"Failed to dismiss",i.classList.remove("hidden")),s.disabled=!1,s.textContent="Dismiss"}})}),e.querySelectorAll(".orphan-checkbox").forEach(t=>{t.addEventListener("change",r=>{const s=r.target,n=parseInt(s.dataset.line||"0",10);s.checked?o.selectedLines.add(n):o.selectedLines.delete(n),j()})}),e.querySelectorAll(".preview-source-btn").forEach(t=>{t.addEventListener("click",r=>{r.preventDefault();const s=r.currentTarget.dataset.docId,n=r.currentTarget.dataset.line;s&&K(s,n?parseInt(n,10):void 0)})})}function Tr(){const e=document.getElementById("bulk-assign-btn"),t=document.getElementById("bulk-dismiss-btn"),r=document.getElementById("bulk-target-input");e==null||e.addEventListener("click",async()=>{const s=r==null?void 0:r.value.trim();if(!s){alert("Please enter a document ID");return}confirm(`Assign ${o.selectedLines.size} orphan(s) to ${s}?`)&&await De(s)}),t==null||t.addEventListener("click",async()=>{confirm(`Dismiss ${o.selectedLines.size} orphan(s)?`)&&await De("dismiss")})}function Ir(){const e=document.getElementById("repo-select");e==null||e.addEventListener("change",t=>{o.selectedRepo=t.target.value,o.selectedLines.clear(),P()})}function Ar(){o.bulkMode=!o.bulkMode,o.bulkMode||o.selectedLines.clear(),j()}function Rr(){const e=o.bulkMode?"Exit bulk mode":"Bulk actions";return`
    <div class="space-y-4 sm:space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
  ...
  `}async function _r(){var e;await Er(),Ir(),(e=document.getElementById("bulk-mode-toggle"))==null||e.addEventListener("click",Ar),o.selectedRepo?await P():(o.loading=!1,j())}function qr(){o.data=null,o.loading=!0,o.error=null,o.successMessage=null,o.bulkMode=!1,o.selectedLines.clear(),ke()}const Br=".document-group, .suggestion-card, .orphan-card, .question-card";let $=-1,ge=null,ne=!1;function X(){return Array.from(document.querySelectorAll(Br))}function ae(e){const t=X();if(t.length===0)return;const r=Math.max(0,Math.min(e,t.length-1));$>=0&&$<t.length&&t[$].classList.remove("keyboard-focused"),$=r;const s=t[$];s.classList.add("keyboard-focused"),s.scrollIntoView({behavior:"smooth",block:"nearest"}),s.setAttribute("tabindex","0"),s.focus()}function jr(){X().length!==0&&($<0?ae(0):ae($+1))}function Hr(){const e=X();e.length!==0&&($<0?ae(e.length-1):ae($-1))}function Dr(){const e=X();if($<0||$>=e.length)return;const t=e[$],r=t.querySelector(".preview-doc-btn, .preview-line-btn, .compare-btn, .sections-btn");if(r){r.click();return}const s=t.querySelector('textarea[name="answer"]');if(s){s.focus();return}const n=t.querySelector("button");n&&n.click()}function zr(){if(ne){ye();return}const e=document.getElementById("document-preview-panel"),t=document.getElementById("merge-preview-panel"),r=document.getElementById("split-preview-panel");if(e&&!e.classList.contains("hidden")){const s=e.querySelector(".close-preview-btn");s==null||s.click();return}if(t&&!t.classList.contains("hidden")){const s=t.querySelector(".close-preview-btn");s==null||s.click();return}if(r&&!r.classList.contains("hidden")){const s=r.querySelector(".close-preview-btn");s==null||s.click();return}Pr()}function Pr(){X().forEach(t=>{t.classList.remove("keyboard-focused"),t.removeAttribute("tabindex")}),$=-1}function Or(){var t,r;if(ne)return;ne=!0;const e=document.createElement("div");e.id="keyboard-help-modal",e.className="fixed inset-0 z-50 flex items-center justify-center bg-black/50",e.setAttribute("role","dialog"),e.setAttribute("aria-modal","true"),e.setAttribute("aria-labelledby","keyboard-help-title"),e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",s=>{s.target===e&&ye()}),(t=document.getElementById("close-help-modal"))==null||t.addEventListener("click",ye),(r=document.getElementById("close-help-modal"))==null||r.focus()}function ye(){const e=document.getElementById("keyboard-help-modal");e&&e.remove(),ne=!1}let ee=!1,O=null;function Fr(e){const t=e.target;if(t.tagName==="INPUT"||t.tagName==="TEXTAREA"||t.tagName==="SELECT"){e.key==="Escape"&&(t.blur(),e.preventDefault());return}if(ee)switch(ee=!1,O&&(clearTimeout(O),O=null),e.key.toLowerCase()){case"d":window.location.hash="#/",e.preventDefault();return;case"r":window.location.hash="#/review",e.preventDefault();return;case"o":window.location.hash="#/organize",e.preventDefault();return}switch(e.key.toLowerCase()){case"j":jr(),e.preventDefault();break;case"k":Hr(),e.preventDefault();break;case"enter":$>=0&&(Dr(),e.preventDefault());break;case"escape":zr(),e.preventDefault();break;case"?":Or(),e.preventDefault();break;case"g":ee=!0,O=window.setTimeout(()=>{ee=!1,O=null},500),e.preventDefault();break}}function Nr(){ge||(ge=Fr,document.addEventListener("keydown",ge))}function Ur(){$=-1}let w=!1;function Qr(e){switch(e){case"/":return Ee();case"/review":return Vt();case"/organize":return kr();case"/orphans":return Rr();default:return Ee()}}function Kr(e){return ve.map(t=>{const r=t.path===e,s="flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors",n=r?"bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200":"text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700";return`<a href="#${t.path}" class="${s} ${n}">
      <span>${t.icon}</span>
      <span>${t.title}</span>
    </a>`}).join("")}function Gr(e){return ve.map(t=>{const r=t.path===e,s="flex items-center space-x-3 px-4 py-3 text-base font-medium transition-colors",n=r?"bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200":"text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700";return`<a href="#${t.path}" class="mobile-nav-link ${s} ${n}">
      <span class="text-xl">${t.icon}</span>
      <span>${t.title}</span>
    </a>`}).join("")}function Wr(){return`
  ...
  `}let ze=null;function Vr(e){e==="/"||e===null?yt():e==="/review"?Xt():e==="/organize"?$r():e==="/orphans"&&qr()}function Jr(e){e==="/"?ft():e==="/review"?Jt():e==="/organize"?wr():e==="/orphans"&&_r(),Ur()}function nt(e){var s;Vr(ze),ze=e,w=!1;const t=((s=ve.find(n=>n.path===e))==null?void 0:s.title)||"Dashboard",r=document.querySelector("#app");r.innerHTML=`
  ...
  `,Yr(),Jr(e)}function Yr(){const e=document.getElementById("mobile-menu-btn");e==null||e.addEventListener("click",()=>{var r,s,n,a;w=!w;const t=document.getElementById("mobile-nav");if(t&&t.classList.toggle("hidden",!w),e){e.setAttribute("aria-expanded",String(w));const i=e.querySelectorAll("svg");(r=i[0])==null||r.classList.toggle("hidden",w),(s=i[0])==null||s.classList.toggle("block",!w),(n=i[1])==null||n.classList.toggle("hidden",!w),(a=i[1])==null||a.classList.toggle("block",w)}}),document.querySelectorAll(".mobile-nav-link").forEach(t=>{t.addEventListener("click",()=>{w=!1})})}Pe.onRouteChange(e=>{nt(e)});Nr();Ve();nt(Pe.getCurrentRoute());
