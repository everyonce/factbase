# Workflow Routing Test Results — Haiku v6
...
|-------|-----|---------|-------|-------|
| **claude-haiku-4-5** | **v6 (this run)** | **10** | **0** | **10/10** |
| claude-haiku-4-5 | v5 | 10 | 0 | 10/10 |
...
The v6 prompts were designed to be harder than v5 — more ambiguous framing, mixed signals, and edge cases. Haiku handled all of them correctly:
...
**Read-only routing (P5, P10):** Both prompts asking "what does the KB say" correctly routed to `search` rather than any write workflow. P10 is particularly tricky — it states a fact ("written by John the Apostle") but then asks what the KB says, making it a read-only query.

**Refresh vs Add (P7):** "New work on the historical Paul recently" correctly identified as `refresh` (update existing content with new external scholarship) rather than `add` (create new documents). The "recently" signal is key.

**Organize (P6):** Correctly identified as `factbase(op=organize)` rather than `workflow(maintain)`. Restructuring/grouping is a distinct operation.

**Maintain vs Correct (P9):** "Accurate and complete?" is a quality audit, not a specific correction claim → `maintain` is correct.

### No regressions from v5

All routing patterns established in v5 hold under harder prompts.
