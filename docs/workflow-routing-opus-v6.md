# Workflow Routing Test Results — Opus v6
...
|-------|-----|---------|-------|-------|
| **claude-opus-4-6** | **v6 (this run)** | **10** | **0** | **10/10** |
| claude-opus-4-6 | v5 | 10 | 0 | 10/10 |
| claude-opus-4-6 | v4 | 5 | 5 | 5/10 |
| claude-opus-4-6 | v1 | 6 | 3 | 6/10 |
...
**Read-only routing (P5, P10):** Both prompts asking "what does the KB say" correctly routed to `search`. P10 is particularly tricky — it states a fact ("written by John the Apostle") but then asks what the KB says, making it a read-only query.
...
All routing patterns established in v5 hold under harder prompts. The fixes from #516 and #517 remain stable across all three models.
